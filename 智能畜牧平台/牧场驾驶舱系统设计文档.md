...
graph TD
    subgraph "前端 (jeecgboot-vue3)"
        A[Dashboard.vue<br>驾驶舱主页面] --> B;
        A -- "包含" --> C[MapMonitor.vue<br>地图核心组件];
        A -- "包含" --> D[StatCards.vue<br>统计卡片组];
        A -- "包含" --> E[RealtimeAlarmList.vue<br>实时告警列表];
    end

    subgraph "后端 (jeecg-module-animal-husbandry)"
        B(DashboardController<br>驾驶舱专用API) -- "调用" --> F[DashboardServiceImpl];
    end
    
    subgraph "数据服务层"
        F -- "聚合查询" --> G[MySQL数据库<br>(ah_animal, ah_device, ah_alarm_record, ah_daily_stats_report)];
        F -- "可选：读取历史" --> H[时序数据库<br>(TDengine)];
    end
...
2.  **地图牲畜数据**: `MapMonitor.vue` 组件加载后，会启动一个定时器（如每15秒），定期调用API刷新所有牲畜在地图上的位置和状态。这是保证“实时性”的关键。
3.  **告警数据**: 告警列表组件也会定期刷新，以展示最新的告警信息。
...
| `getRecentAlarms` | GET | `/recent-alarms`| 获取最新的N条告警记录。 | 高 |
...
*   **设计要点**:
    ...
    *   **数据轮询**: 在 `Dashboard.vue` 的 `onMounted` 生命周期钩子中启动定时器，定期调用 `getMapAnimalData` 和 `getRecentAlarms` API，并在 `onUnmounted` 中清除定时器，防止内存泄漏。
