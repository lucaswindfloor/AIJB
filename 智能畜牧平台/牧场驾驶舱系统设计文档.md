...
*   **越界告警**: 后端定时任务分析牲畜实时位置，当发现有牲畜坐标点位于其不应在的围栏区域之外时，自动生成“越界告警”记录。
...
*   **设计要点**:
    ...
    *   **数据轮询**: 在 `Dashboard.vue` 的 `onMounted` 生命周期钩子中启动定时器，定期调用 `getMapAnimalData`、`getRecentAlarms` 和 `getFences` API，并在 `onUnmounted` 中清除定时器，防止内存泄漏。
